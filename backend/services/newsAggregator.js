...
class NewsAggregator {
  ...
  async getArticles({ category = "", page = 1, limit = 10 } = {}) {
    const cacheKey = category ? `feed:${category}` : "feed:all";
    ...
    if (articles.length > 0) {
      ...
      cacheService
        .cacheArticles(cacheKey, articles.slice(0, 30))
        .catch(() => {});
    }
  ...
  async _fetchCategory(category) {
    ...
    if (worldNewsService.isAvailable()) {
      try {
        ...
        console.warn(
          `[Aggregator] World News API failed for ${category}:`,
          e.message
        );
      }
    }
    ...
    if (currentsService.isAvailable()) {
      try {
        ...
        console.warn(
          `[Aggregator] Currents failed for ${category}:`,
          e.message
        );
      }
    }
  ...
  async _getPoolArticles() {
    ...
    if (this._articlePool.size < 15) {
      ...
      if (worldNewsService.isAvailable()) {
        ...
        fetches.push(
          worldNewsService.fetchIndiaNews({ number: 30 }).catch((e) => {
        ...
        // Global top stories
        fetches.push(
          worldNewsService.fetchTopGlobalNews().catch((e) => {
  ...
  async refreshAllCaches() {
    ...
    if (worldNewsService.isAvailable()) {
      ...
      try {
        ...
        if (globalNews.length > 0) {
          ...
          successCount++;
          console.log(
            `[Aggregator] Cached ${globalNews.length} global top stories`
          );
        }
      } catch (e) {
    ...
    console.log(
      `[Aggregator] Cache refresh done: ${successCount}/${
        ALL_CATEGORIES.length + 2
      } OK | Pool: ${this._articlePool.size} articles`
    );
  }
