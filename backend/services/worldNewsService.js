...
class WorldNewsService {
  ...
  isAvailable() {
    ...
    if (this._quotaExhausted) {
      ...
      if (
        this._quotaExhaustedAt &&
        Date.now() - this._quotaExhaustedAt > 60 * 60 * 1000
      ) {
        this._quotaExhausted = false;
        this._quotaExhaustedAt = null;
  ...
  async fetchIndiaNews({
    ...
    try {
      ...
      params["earliest-publish-date"] = sevenDaysAgo
        .toISOString()
        .split("T")[0];
      ...
      console.log(
        `[WorldNews] Fetched ${news.length} India articles (available: ${
  ...
  async fetchByCategory(category, { number = 15 } = {}) {
    ...
    try {
      ...
      params["earliest-publish-date"] = threeDaysAgo
        .toISOString()
        .split("T")[0];
